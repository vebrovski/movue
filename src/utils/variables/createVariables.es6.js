 ...
 */
export const createList = (obj = {}, pre = '', post = '', unit = '') => {
  const props = {};
 ...
 * @param {*} obj Provided object to transition into SCSS Variables
 */
export const createMediaList = (obj = {}) => {
  const media = {};
 ...
 */
export const getBreakpointsConfig = (obj = {}) => {
  const media = {};
  const keys = Object.keys(obj).sort((a, b) => obj[a] - obj[b]);
  const values = keys.map((key) => obj[key]);
 ...
 */
export const assignToCustomProps = (keys = [], pre = '', post = '') => {
  const props = {};
  keys.forEach((key) => {
